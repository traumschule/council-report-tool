...
import { spendigBudgetBarType, WGspedingType, spendingBudgetPieType, OverallType } from "@/hooks/types";

import { uploadImage } from "@/helpers";
import { councilReportWithMediaStatus, councilReportWithOutMediaStatus } from "@/config";

function SpendingBarChart({ data, title, id, type }: { data: spendigBudgetBarType[]; title: string, id: string, type: boolean }) {
    ...
    <div className="p-2">
      <div id={type ? id + "-joy" : id + "-usdt"}>
        <BarChart id={id} width={900} height={250} data={data}>
          ...
          {type ? (
            <>
              <Bar dataKey="prevSpendingOfJoy" fill="#8884d8" name={title + " (prev term ), JOY"} />
              <Bar dataKey="currentSpendingOfJoy" fill="#ff6600" name={title + " (current term), JOY"} />
            </>
          ) : (
            <>
              <Bar dataKey="prevSpendingOfUsd" fill="#8884d8" name={title + " (prev term ), USDT"} />
              <Bar dataKey="currentSpendingOfUsd" fill="#ff6600" name={title + " (current term ), USDT"} />
            </>
          )}
        </BarChart>
      </div>
...
function SpendingPieChart({ title, id, data, color }: { title: string, id: string, data: Array<spendingBudgetPieType>, color: Array<string> }) {
  if (data.length === 0) return <></>;
  return (
    <div id={id}>
      <div style={{ display: "flex" }}>
        <PieChart width={350} height={350}>
          <Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={120} fill="#82ca9d" labelLine={false}>
            {data.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={color[index % color.length]} />
            ))}
          </Pie>
        </PieChart>
        <div className="mr-3 mt-4" style={{ columnCount: id == "overallPieChart" ? 1 : 2, columnGap: "40px" }}>
          {
            data.map((a, index) =>
              <div key={index} style={{ display: "flex", alignItems: "center" }}>
                <div className="mr-1" style={{ height: 10, width: 10, backgroundColor: color[index] }} />
                <p> <span style={{ fontWeight: 300 }}>{a.name} </span> {a.value + " %"}</p></div>)
          }
        </div>
      </div>
...
export default function Council() {
  ...
  const wgPieColors = ['#ff4000', '#ff8000', '#ffff00', '#ff0080', '#00ff00', '#00ffbf', '#00bfff', '#0040ff', '#4000ff']
  const graphTypes = [
    'overallPieChart',
    'wgPieChart',
    'overallBudget-joy',
  ...
  const generate = async () => {
    ...
    const [report4] = await Promise.all([
      generateReport4(api, data[activeCouncil - 1].id, startBlock, endBlock, prevEMA, curEMA, storageFlag),
    ]);
    ...
    setReport4(report4);
    setFileName("Council_Report_" + moment(report4.general.startBlock.timeStamp).format('DD-MMM-YYYY') + ".md");
    setLoading(false);
  }
  ...
  const generateWGBudgetPieChartData = (wgSpending: WGspedingType) => {
    ...
    type wgSpendingKeys = keyof typeof wgSpending;
    const total = Object.keys(wgSpending)
      .reduce((a, b) => a + wgSpending[b as wgSpendingKeys].currentSpendingOfJoy, 0);
    Object.keys(wgSpending).map((_type) => {
      let tmp = {
        name: _type,
        value: decimalAdjust(100 * wgSpending[_type as wgSpendingKeys].currentSpendingOfJoy / total)
      }
  ...
  const generateOverallBudgetPieChartData = (overallBudget: OverallType) => {
    ...
    let total = 0;
    Object.keys(overallBudget).map((_type) => {
      if (_type != "grandTotal" && _type != "fees" && _type != "fundingProposal") {
    ...
    Object.keys(overallBudget).map((_type) => {
      if (_type != "grandTotal" && _type != "fees" && _type != "fundingProposal") {
        let tmp = {
          ...
          value: decimalAdjust(100 * overallBudget[_type as overallTypes].currentSpendingOfJoy / total)
        }
        chartData.push(tmp);
      }
    });
    ...
    Object.keys(councilReportData).map((_type) => {
      if (typeof (councilReportData[_type as reportKeys]) != "object" && _type != "projectInflations" && councilReportData[_type as reportKeys] != undefined && _type != "composition") {
        ...
        Object.keys(projectInflations).map((_infaltion) => {
          const tmp_inflation = projectInflations[_infaltion as inflationType] as {
            term: number;
            mintedToken: number;
            inflation: number;
          }
        ...
        if (councilReportData[_type as reportKeys] != undefined && _type != "projectInflations" && _type != "composition") {
          writeCouncilReport(councilReportData[_type as reportKeys], alias + "_" + _type);
        }
      }
    })
  }
  ...
  const exportGraphImage = async () => {
    ...
    element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(councilReport));
  ...
  const exportCouncilReport = async () => {
    if (fileName == "")
      return;
    if (storageFlag)
      councilReport = councilReportWithMediaStatus
    else
      councilReport = councilReportWithOutMediaStatus
  ...
  return (
    <div className="prose max-w-3xl m-auto mt-4">
      <div className="rounded-sm p-2 mt-4 border-2 border-[#fff]">
        ...
        <div className="mt-4" />
        <SpendingPieChart title="Total Spending" data={overallPie} id="overallPieChart" color={overallPieColors} />
        <SpendingPieChart title="Total Spending" data={wgPie} id="wgPieChart" color={wgPieColors} />
        <SpendingBarChart title="Total Spending" data={overallBar} id="overallBudget" type={true} />
        <SpendingBarChart title="Total Spending" data={overallBar} id="overallBudget" type={false} />
        <SpendingBarChart title="Total Spending" data={wgBar} id="wgBudget" type={true} />
        <SpendingBarChart title="Total Spending" data={wgBar} id="wgBudget" type={false} />
      </div>
    </div >
  );
}
