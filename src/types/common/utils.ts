...
export const isFunction = (something: unknown): something is CallableFunction =>
...
export const isRecord = (something: unknown): something is Obj =>
...
export const toNumber = (value: any): number =>
  value?.toNumber?.() ?? (isNumber(value) ? value : NaN);
...
export const objectEquals = <T extends Obj>(
  ...
  const equalsOption = {
    checkExtraKeys,
    depth: isNumber(depth) ? depth - 1 : depth,
  };
...
export const intersperse = <T, S>(
  ...
  list.length < 2
    ...
    : [
        ...
        ...list
          .slice(1)
          .flatMap((item, index) => [toSeparator(index, list), item]),
      ];
...
export const flatten = <T>(nested: (T | T[])[]) =>
  ([] as T[]).concat(...nested);

export const asArray = <T>(item: undefined | T): T[] =>
  isDefined(item) ? [item] : [];
